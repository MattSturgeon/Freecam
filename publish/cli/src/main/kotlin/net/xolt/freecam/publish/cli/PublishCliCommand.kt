...
internal class PublishCliCommand(
    ...
    override suspend fun run() {
        Logger.configure {
            level = logLevel
            sink = { message ->
                // Use Clikt's `echo` for smart colored output,
                // system-specific line endings, etc
                echo(message, err = level.isStderr)
            }
            if (ghaAnnotations) decorate(GitHubAnnotationsLogDecorator)
            else decorate(AnsiColorLogDecorator)
        }
